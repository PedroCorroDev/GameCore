...
if (!empty($nombre)&&!empty($apellidos)&&!empty($email)&&!empty($telefono)&&!empty($fecha_nacimiento)&&!empty($usuario)&&!empty($contrasenya)){   
  include  "comprobacion.php";
    ...
    header("Location: " . dirname($_SERVER["PHP_SELF"]) . "/../views/registro.php?nombre=".$errores['nombre']."&apellidos=".$errores['apellidos']."&email=".$errores['email']."&telefono=".$errores['telefono']."&fecha_nacimiento=".$errores['fecha_nacimiento']."&usuario=".$errores['usuario']."&contrasenya=".$errores['contrasenya']."&voltear=1");
}
?>
